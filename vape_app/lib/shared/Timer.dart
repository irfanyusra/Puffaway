...
class _timerState extends State<timer> {
  ...
  Widget build(BuildContext context) {
    ...
    final logs = Provider.of<List<Log>>(context)??[];
      ...
      if(diff.inDays==1){//vape free for 1 day
        _fcm.subscribeToTopic("day1");

      }else if(diff.inDays==2){//vape free for 2 days
        _fcm.unsubscribeFromTopic("day1");
        _fcm.subscribeToTopic("day2");

      }else if(diff.inDays==3){//unsubscribe from day 2 message
        _fcm.unsubscribeFromTopic("day2");

      }else if(diff.inDays==5){//vape free for 5 days
        _fcm.subscribeToTopic("day5");

      }else if(diff.inDays==6){
        _fcm.unsubscribeFromTopic("day5");

      }else if(diff.inDays==7){//vape free for 1 week
